...
constexpr void buildRec(std::vector<std::pair<double, double> > &samples, double deltaX, double llamb, double rlamb,
                        int k) {
...
uint64_t queryFulcrums(const std::vector<uint64_t> &fulcs, uint64_t hash, uint64_t bucketCount) {
    ...
    uint64_t index = z >> 64;
    uint64_t part = z;
    uint64_t v1 = (uint64_t((fulcs[index + 0] * __uint128_t(bucketCount)) >> 32) * __uint128_t(uint64_t(-1) - part)) >>
                  64;
...
class OptimalBucketFunction {
    ...
    public:
        class Instance {
            private:
                ...
                Instance(std::shared_ptr<std::vector<uint64_t> > fulcrums,
                         uint64_t bucket_count, uint64_t load_factor): fulcrums(move(fulcrums)),
        ...
        OptimalBucketFunction(uint64_t k): fulcrums(
            std::make_shared<std::vector<uint64_t> >(optimal_bucket_function::getBucketFunctionFulcrums(k, 1000))) {
        }
