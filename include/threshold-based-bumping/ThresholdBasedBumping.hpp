...
constexpr double expected_overflow(uint64_t _k, double bucket_size) {
	double p = std::exp(-bucket_size);
	double res = 0;
	double remaining = 1 - p;
	for (uint64_t i = 1; i < _k; i++) {
		p *= bucket_size / i;
		remaining -= p;
		res += p * i;
	}
	return bucket_size - (res + remaining * _k);
}
	  ...
	  build(std::vector<Hash128> keys, F &&build_phf, Filter::Builder filter) {
		...
		for (uint64_t i = 0; offset != total_buckets; i++) {
			uint64_t cur_buckets =
			  std::min(total_buckets - offset,
			         uint64_t(std::ceil(keys.size() / overload_bucket_size)));
			...
			for (uint64_t j = 0; j < cur_buckets; j++) {
				...
				if (bucket.size() <= _k) {
					...
					tidx =
					  std::ranges::upper_bound(avail_thresholds,
					    bucket[_k].fingerprint) - avail_thresholds.begin();
				...
				if (tidx >= 2 && bound == perfect && bound != bucket.begin() &&
				  prev(bound)->fingerprint < avail_thresholds[tidx-2]) {
					...
					while (upper != bucket.end() &&
					  (tidx == n_thresholds ||
					   upper->fingerprint < avail_thresholds[tidx])) {
